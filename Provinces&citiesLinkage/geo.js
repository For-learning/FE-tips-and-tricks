...
/*
说明：
    如果数据从数据库中读取出来需要事先初始化省市联动，因为省市联动一启动，省份信息会初始化到第一个select中，这个时候让第一个select中的需要选中的省份处于选中状态，在运行一下change(1)，这个时候城市的信息就会初始化到第二个select中，就可以根据需要将第二个select中的内容置于选中状态了。
*/
// 声明一个Dsy类，用来存放所有省市的信息。
function Dsy() {
    this.Items = {};
}
// 当Dsy创建对象后，用来向里面添加值
...
function change(v) {
    var str = "0";
    //change(0)的时候默认执行一遍，使得s1中有默认的初始化值。
    for (i = 0; i < v; i++) {
        // ##多个需要改的地方##
        str += ("_" + (document.getElementById(s[i]).selectedIndex - 1)); //0_0
    }
    var ss = document.getElementById(s[v]); // 当v=0的时候，ss为select：s1
    with(ss) {
        length = 0;
        options[0] = new Option(opt0[v], opt0[v]); //是ss.options[0],得到第一个元素
        // 这里如果v是0的话就不执行了,只有第二select即选择城市的时候才走这个方法，这个方法是选择地区的。
...
function change_a(v) {
    ...
    for (i = 0; i < v; i++) {
        // ##多个需要改的地方##
        str += ("_" + (document.getElementById(a[i]).selectedIndex - 1)); //0_0
    }
    var ss = document.getElementById(a[v]); // 当v=0的时候，ss为select：s1
    with(ss) {
        length = 0;
        options[0] = new Option(opt0_a[v], opt0_a[v]); //是ss.options[0],得到第一个元素
        // 这里如果v是0的话就不执行了。
        ...
        // ++v,先使用v的值进行比较之后再进行赋值。
        if (++v < s.length) { change_a(v); }
    }
}
// ##############################多个的时候##############################

// 一开始的时候可以使得省市联动有初始值，这个方法可以不用。
...
// 初始化，页面上有三个select，id分别是："s1", "s2", "s3"。
var s = ["s1", "s2", "s3"];
var opt0 = ["全部", "全部", "县、市、区"];
// 用来启动省市联动
function setup() {
    for (i = 0; i < s.length - 1; i++)
    // 给每一个select默认绑定一个change事件,给s1绑定change(1)事件，给s2绑定change(2)事件。
...
// ##############################多个的时候##############################
// 初始化，页面上有三个select，id分别是："a1", "a2", "a3"
var a = ["a1", "a2", "a3"];
var opt0_a = ["全部", "全部", "县、市、区"];
// 用来启动省市联动
function setup_a() {
